...
export default function SelectedCardDetail({
  ...
  const handleMarkStatusAppointment = async (a: string, status: string) => {
    try {
      const response = await axiosInstance.patch(
        `appointments/mark-status/${a}?status=${status}`
      );
  ...
  const getViAppointmentType = (type: string) => {
    switch (type) {
  ...
  const currentStep = getStepBySelectedStepDetail(
    orderSteps,
    selectedStepDetail
  );
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="text-white">
                <h3 className="text-2xl font-bold leading-tight">
                  {currentStep?.treatmentStep.stepName}
                </h3>
                <p className="text-blue-100 font-medium">
                  Bước {currentStep?.treatmentStep.stepOrder}
                </p>
              </div>
            </div>
            <button
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-6 border border-gray-100">
              <p className="text-gray-700 leading-relaxed text-lg">
                {currentStep?.treatmentStep.description}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200">
                ...
                <p className="text-2xl font-bold text-green-900">
                  {currentStep?.totalAmount?.toLocaleString()} VNĐ
                </p>
              </div>

              <div className="bg-gradient-to-br from-yellow-50 to-gray-100 rounded-xl p-6 border border-gray-200">
                <div className="flex items-center mb-2">
                ...
                <span
                  className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${
                    currentStep?.paymentStatus === PAYMENT_COMPLETED
                      ? "bg-green-100 text-green-800 border border-green-200"
                      : currentStep?.paymentStatus === PAYMENT_CANCELLED
                      ? "bg-orange-100 text-orange-800 border border-orange-200"
                      : currentStep?.paymentStatus === PAYMENT_FAILED
                      ...
                      <div className="bg-gradient-to-r from-gray-50 to-blue-50 px-6 py-4 border-b border-gray-100">
                        <div className="flex items-center justify-between">
                          <h5 className="text-lg font-semibold text-gray-900">
                            {getViAppointmentType(appointment.type??"")}
                          </h5>
                          <span
                      ...
                      <div className="p-6 space-y-4">
                        ...
                        <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-100">
                          {order.treatmentService?.name === "IVF" &&
                            currentStep?.treatmentStep.stepOrder ===
                              STEP_TRANSFER && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
